...
fprintf('IoU: %.4f\n', IoU);
fprintf('Dice: %.4f\n', Dice);
%% 11. Codex-readable runtime export
try
    project_root = fileparts(mfilename('fullpath'));
    codex_run_dir = fullfile(project_root, 'codex_runs');
    if ~exist(codex_run_dir, 'dir')
        mkdir(codex_run_dir);
    end
    run_id = datestr(now, 'yyyymmddTHHMMSSFFF');

    codex_run = struct();
    codex_run.schema_version = '1.0';
    codex_run.run_id = run_id;
    codex_run.generated_at = char(datetime('now', 'TimeZone', 'UTC', 'Format', 'yyyy-MM-dd''T''HH:mm:ss.SSS''Z'''));
    codex_run.source = 'HDSPdebug.m';
    codex_run.parameters = struct( ...
        'Nx', Nx, ...
        'Ny', Ny, ...
        'Nz', Nz, ...
        'Lx', Lx, ...
        ...
        'dz', dz, ...
        'f0', f0, ...
        'z_target_dist', z_target_dist, ...
        'phase_refine_mode', phase_refine_mode, ...
        'iasa_epoch', iasa_epoch, ...
        'pdms_thickness', pdms_thickness, ...
        'cavitation_limit', cavitation_limit, ...
        'target_median_pressure', target_median_pressure, ...
        'exposure_time', exposure_time, ...
        'cooling_time', cooling_time);
    codex_run.metrics = struct( ...
        'best_corr', best_corr, ...
        'SSIM_val', SSIM_val, ...
        'NMSE', NMSE, ...
        'Energy_Efficiency', Energy_Efficiency, ...
        'asm_python_pcc', asm_python_pcc, ...
        'asm_python_ssim', asm_python_ssim, ...
        'asm_python_nmse', asm_python_nmse, ...
        'asm_iasa_pcc', asm_iasa_pcc, ...
        'asm_iasa_ssim', asm_iasa_ssim, ...
        'asm_iasa_nmse', asm_iasa_nmse, ...
        'asm_kwave_corr', asm_kwave_corr, ...
        'asm_kwave_nmse', asm_kwave_nmse, ...
        'asm_python_kwave_corr', asm_python_kwave_corr, ...
        'asm_python_kwave_nmse', asm_python_kwave_nmse, ...
        'board_exit_asm_pcc', board_exit_asm_pcc, ...
        'board_exit_kwave_corr', board_exit_kwave_corr, ...
        'exit_amp_cv', exit_amp_cv, ...
        'exit_amp_min_ratio', exit_amp_min_ratio, ...
        ...
        'local_std_amp_corr', local_std_amp_corr, ...
        'edge_dist_amp_corr', edge_dist_amp_corr, ...
        'exit_amp_model_r2', exit_amp_model_r2, ...
        'edge_amp_mean', edge_amp_mean, ...
        'flat_amp_mean', flat_amp_mean, ...
        'T_max_real', T_max_real, ...
        'cured_coverage', cured_coverage, ...
        'IoU', IoU, ...
        'Dice', Dice, ...
        'over_cure_ratio', over_cure_ratio, ...
        'under_cure_ratio', under_cure_ratio);
    codex_run.artifacts = struct( ...
        'transport_dir', transport_dir, ...
        'target_for_python', export_path, ...
        'python_phase_init', import_path);
    codex_run.analysis_hint = struct( ...
        'next_command', 'python tools/analyze_codex_run.py --run-json codex_runs/hdsp_latest_run.json', ...
        'metric_profiles', fullfile(project_root, 'codex_analysis', 'metric_profiles.json'));

    codex_json = jsonencode(codex_run);
    latest_path = fullfile(codex_run_dir, 'hdsp_latest_run.json');
    archive_path = fullfile(codex_run_dir, ['hdsp_run_' run_id '.json']);
    fid = fopen(latest_path, 'w');
    fwrite(fid, codex_json, 'char');
    fclose(fid);
    fid = fopen(archive_path, 'w');
    fwrite(fid, codex_json, 'char');
    fclose(fid);
    fprintf('Codex runtime JSON: %s\n', latest_path);
    analysis_script = fullfile(project_root, 'tools', 'analyze_codex_run.py');
    analysis_output = fullfile(project_root, 'codex_analysis', 'latest_run_analysis.json');
    analysis_command = sprintf('python "%s" --run-json "%s" --output "%s"', analysis_script, latest_path, analysis_output);
    analysis_status = system(analysis_command);
    if analysis_status == 0
        fprintf('Codex proposal-only analysis: %s\n', analysis_output);
    else
