...
codex_run_dir = os.path.join(project_root, "codex_runs")
os.makedirs(codex_run_dir, exist_ok=True)
run_id = datetime.now(timezone.utc).strftime("%Y%m%dT%H%M%SZ")
layer_values = dithered_layers[mask_np > 0.5]
summary = {
    "schema_version": "1.0",
    "run_id": run_id,
    ...
    "metrics": {
        ...
        "phase_bias": float(phase_bias_final.item()),
        "phase_step": float(phase_step),
        "layer_min": int(np.min(layer_values)) if layer_values.size else None,
        "layer_max": int(np.max(layer_values)) if layer_values.size else None,
        "layer_mean": float(np.mean(layer_values)) if layer_values.size else None,
    },
    "artifacts": {
        "phase_mat": output_file,
    },
}
latest_path = os.path.join(codex_run_dir, "pann_latest_run.json")
archive_path = os.path.join(codex_run_dir, f"pann_run_{run_id}.json")
...
with open(archive_path, "w", encoding="utf-8") as handle:
    json.dump(summary, handle, ensure_ascii=False, indent=2)
    handle.write("\n")
print(f"[OK] Codex-readable PANN run summary written to: {latest_path}")

analysis_output = os.path.join(project_root, "codex_analysis", "pann_latest_run_analysis.json")
analysis_command = f'python "{os.path.join(project_root, "tools", "analyze_codex_run.py")}" --run-json "{latest_path}" --output "{analysis_output}"'
analysis_status = os.system(analysis_command)
if analysis_status == 0:
    print(f"[OK] Codex proposal-only PANN analysis written to: {analysis_output}")
else:
    print(f"[WARN] Codex PANN analysis command failed with status: {analysis_status}")
