...
local_exit_diagnostic = analyze_local_exit_field( ...
    exit_result.local_exit_complex, exit_result.local_exit_comp_plus, exit_result.local_exit_comp_minus, ...
    holo_phase, circle_mask_board, ...
...
local_comp_plus_repaired = make_idealized_exit_field(local_exit_diagnostic.comp_plus_complex, circle_mask_board, 1.0);
local_comp_minus_repaired = make_idealized_exit_field(local_exit_diagnostic.comp_minus_complex, circle_mask_board, 1.0);
...
local_comp_plus_asm = propagate_exit_field_asm(local_comp_plus_repaired, H_forward, center_idx, Nx_pad, Ny_pad);
local_comp_plus_asm_norm = local_comp_plus_asm / (max(local_comp_plus_asm(:)) + eps);
local_comp_minus_asm = propagate_exit_field_asm(local_comp_minus_repaired, H_forward, center_idx, Nx_pad, Ny_pad);
local_comp_minus_asm_norm = local_comp_minus_asm / (max(local_comp_minus_asm(:)) + eps);
fig_local = figure('Color', 'w', 'Position', [80, 80, 1650, 920]);
...
nexttile; imagesc(x * 1e3, y * 1e3, angle(local_exit_diagnostic.comp_plus_complex)); axis image; colormap(gca, hsv); colorbar; clim([-pi, pi]); title('Local exit phase +kdz');
nexttile; imagesc(x * 1e3, y * 1e3, angle(local_exit_diagnostic.comp_minus_complex)); axis image; colormap(gca, hsv); colorbar; clim([-pi, pi]); title('Local exit phase -kdz');
...
nexttile; imagesc(x * 1e3, y * 1e3, local_comp_plus_asm_norm); axis image; colormap(gca, turbo); colorbar; title(sprintf('+kdz repaired PCC %.4f', local_exit_diagnostic.metrics.comp_plus.repaired_fixed_pcc));
nexttile; imagesc(x * 1e3, y * 1e3, local_comp_minus_asm_norm); axis image; colormap(gca, turbo); colorbar; title(sprintf('-kdz repaired PCC %.4f', local_exit_diagnostic.metrics.comp_minus.repaired_fixed_pcc));
...
plot(offset_mm, [exit_phase_scan.records.repaired_signed_best_pcc], '^-', 'LineWidth', 1.8);
grid on; xlabel('Probe offset after max board exit (mm)'); ylabel('PCC to target');
legend('actual fixed z', 'repaired fixed z', 'actual best z', 'repaired same best z', 'repaired signed best z', 'Location', 'best');
...
fprintf('Best repaired ASM probe offset: %.2f mm | same scan PCC %.4f | signed PCC %.4f at %.2f mm (%s) | phase best=%s | coh %.4f | RMS %.4f rad\n', ...
    exit_phase_scan.best_repaired.summary.offset_mm, ...
    exit_phase_scan.best_repaired.summary.repaired_asm_best_pcc, ...
    exit_phase_scan.best_repaired.summary.repaired_signed_best_pcc, ...
    exit_phase_scan.best_repaired.summary.repaired_signed_best_z_mm, ...
    exit_phase_scan.best_repaired.summary.repaired_signed_phase_sign, ...
...
fprintf('Best phase-match probe offset: %.2f mm | same scan PCC %.4f | signed PCC %.4f at %.2f mm (%s) | phase best=%s | coh %.4f | RMS %.4f rad\n', ...
    exit_phase_scan.best_phase.summary.offset_mm, ...
    exit_phase_scan.best_phase.summary.repaired_asm_best_pcc, ...
    exit_phase_scan.best_phase.summary.repaired_signed_best_pcc, ...
    exit_phase_scan.best_phase.summary.repaired_signed_best_z_mm, ...
    exit_phase_scan.best_phase.summary.repaired_signed_phase_sign, ...
...
fprintf('Local exit raw: same PCC %.4f | signed PCC %.4f at %.2f mm (%s) | phase best=%s | coh %.4f | RMS %.4f rad\n', ...
    local_exit_diagnostic.metrics.raw.repaired_best_pcc, ...
    local_exit_diagnostic.metrics.raw.repaired_signed_best_pcc, ...
    local_exit_diagnostic.metrics.raw.repaired_signed_best_z_mm, ...
    local_exit_diagnostic.metrics.raw.repaired_signed_phase_sign, ...
...
fprintf('Local exit comp +kdz: same PCC %.4f | signed PCC %.4f at %.2f mm (%s) | phase best=%s | coh %.4f | RMS %.4f rad\n', ...
    local_exit_diagnostic.metrics.comp_plus.repaired_best_pcc, ...
    local_exit_diagnostic.metrics.comp_plus.repaired_signed_best_pcc, ...
    local_exit_diagnostic.metrics.comp_plus.repaired_signed_best_z_mm, ...
    local_exit_diagnostic.metrics.comp_plus.repaired_signed_phase_sign, ...
    local_exit_diagnostic.metrics.comp_plus.best_sign, ...
    local_exit_diagnostic.metrics.comp_plus.best_coherence, ...
    local_exit_diagnostic.metrics.comp_plus.best_rms_rad);
fprintf('Local exit comp -kdz: same PCC %.4f | signed PCC %.4f at %.2f mm (%s) | phase best=%s | coh %.4f | RMS %.4f rad\n', ...
    local_exit_diagnostic.metrics.comp_minus.repaired_best_pcc, ...
    local_exit_diagnostic.metrics.comp_minus.repaired_signed_best_pcc, ...
    local_exit_diagnostic.metrics.comp_minus.repaired_signed_best_z_mm, ...
    local_exit_diagnostic.metrics.comp_minus.repaired_signed_phase_sign, ...
    local_exit_diagnostic.metrics.comp_minus.best_sign, ...
    local_exit_diagnostic.metrics.comp_minus.best_coherence, ...
    local_exit_diagnostic.metrics.comp_minus.best_rms_rad);
...
local_exit_comp_plus = local_exit_complex;
local_exit_comp_plus(aperture_mask) = local_exit_complex(aperture_mask) .* ...
    exp(1i * (2 * pi * f0 / c_water) .* common_plane_shift_voxels(aperture_mask) .* dz);
local_exit_comp_minus = local_exit_complex;
local_exit_comp_minus(aperture_mask) = local_exit_complex(aperture_mask) .* ...
    exp(-1i * (2 * pi * f0 / c_water) .* common_plane_shift_voxels(aperture_mask) .* dz);
...
result.local_exit_comp_plus = local_exit_comp_plus;
result.local_exit_comp_minus = local_exit_comp_minus;
...
for idx = 1:n_planes
    ...
    repaired_opposite = make_idealized_exit_field(conj(p_complex), aperture_mask, 1.0);
    repaired_opposite_asm = propagate_exit_field_asm(repaired_opposite, H_forward, center_idx, Nx_pad, Ny_pad);
    repaired_opposite_asm_norm = repaired_opposite_asm / (max(repaired_opposite_asm(:)) + eps);
    repaired_opposite_metrics = calc_image_metrics(repaired_opposite_asm_norm, target_norm);
    repaired_opposite_focus_scan = scan_exit_field_asm_focus( ...
        repaired_opposite, Kz, propagating, asm_focus_scan_distances, center_idx, Nx_pad, Ny_pad, target_norm);
    ...
    records(idx) = build_exit_phase_record( ...
        ...
        repaired_opposite_metrics, actual_focus_scan, repaired_focus_scan, repaired_opposite_focus_scan, exit_vals);

    if records(idx).repaired_signed_best_pcc > best_repaired_pcc
        best_repaired_pcc = records(idx).repaired_signed_best_pcc;
...
function diagnostic = analyze_local_exit_field(local_exit_raw, local_exit_comp_plus, local_exit_comp_minus, ...
    holo_phase, aperture_mask, ...
...
comp_plus = summarize_local_exit_case( ...
    local_exit_comp_plus, holo_phase, aperture_mask, H_forward, Kz, propagating, ...
    asm_focus_scan_distances, center_idx, Nx_pad, Ny_pad, target_norm, amp_threshold_ratio);
comp_minus = summarize_local_exit_case( ...
    local_exit_comp_minus, holo_phase, aperture_mask, H_forward, Kz, propagating, ...
    asm_focus_scan_distances, center_idx, Nx_pad, Ny_pad, target_norm, amp_threshold_ratio);

diagnostic = struct();
diagnostic.raw_complex = local_exit_raw;
diagnostic.comp_plus_complex = local_exit_comp_plus;
diagnostic.comp_minus_complex = local_exit_comp_minus;
diagnostic.metrics = struct('raw', raw, 'comp_plus', comp_plus, 'comp_minus', comp_minus);
end
...
repaired_opposite = make_idealized_exit_field(conj(exit_complex), aperture_mask, 1.0);
...
fixed_opposite_asm = propagate_exit_field_asm(repaired_opposite, H_forward, center_idx, Nx_pad, Ny_pad);
fixed_opposite_asm = fixed_opposite_asm / (max(fixed_opposite_asm(:)) + eps);
fixed_opposite_metrics = calc_image_metrics(fixed_opposite_asm, target_norm);
focus_scan = scan_exit_field_asm_focus( ...
    repaired, Kz, propagating, asm_focus_scan_distances, center_idx, Nx_pad, Ny_pad, target_norm);
focus_scan_opposite = scan_exit_field_asm_focus( ...
    repaired_opposite, Kz, propagating, asm_focus_scan_distances, center_idx, Nx_pad, Ny_pad, target_norm);
...
summary.repaired_opposite_fixed_pcc = fixed_opposite_metrics.pcc;
summary.repaired_opposite_best_pcc = focus_scan_opposite.best_metrics.pcc;
summary.repaired_opposite_best_z_mm = focus_scan_opposite.best_z_mm;
if summary.repaired_opposite_best_pcc > summary.repaired_best_pcc
    summary.repaired_signed_phase_sign = "opposite";
    summary.repaired_signed_best_pcc = summary.repaired_opposite_best_pcc;
    summary.repaired_signed_best_z_mm = summary.repaired_opposite_best_z_mm;
else
    summary.repaired_signed_phase_sign = "same";
    summary.repaired_signed_best_pcc = summary.repaired_best_pcc;
    summary.repaired_signed_best_z_mm = summary.repaired_best_z_mm;
end
...
record = struct( ...
    ...
    'repaired_opposite_asm_pcc', 0, ...
    'repaired_opposite_asm_ssim', 0, ...
    'repaired_opposite_asm_nmse', 0, ...
    'repaired_opposite_asm_best_pcc', 0, ...
    'repaired_opposite_asm_best_z_mm', 0, ...
    'repaired_signed_phase_sign', "", ...
    'repaired_signed_best_pcc', 0, ...
    'repaired_signed_best_z_mm', 0, ...
    'exit_amp_cv', 0, ...
...
record.repaired_opposite_asm_pcc = repaired_opposite_metrics.pcc;
record.repaired_opposite_asm_ssim = repaired_opposite_metrics.ssim;
record.repaired_opposite_asm_nmse = repaired_opposite_metrics.nmse;
record.repaired_opposite_asm_best_pcc = repaired_opposite_focus_scan.best_metrics.pcc;
record.repaired_opposite_asm_best_z_mm = repaired_opposite_focus_scan.best_z_mm;
if record.repaired_opposite_asm_best_pcc > record.repaired_asm_best_pcc
    record.repaired_signed_phase_sign = "opposite";
    record.repaired_signed_best_pcc = record.repaired_opposite_asm_best_pcc;
    record.repaired_signed_best_z_mm = record.repaired_opposite_asm_best_z_mm;
else
    record.repaired_signed_phase_sign = "same";
    record.repaired_signed_best_pcc = record.repaired_asm_best_pcc;
    record.repaired_signed_best_z_mm = record.repaired_asm_best_z_mm;
end
