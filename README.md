...
主验证脚本：`phase_board_exit_repair_validation.m`

该脚本按当前主脚本的相位板构建逻辑生成离散板层，并执行两次仿真：

1. 短 z 轴真实相位板仿真：只传播到板出口面，解调并保存 `p_exit_complex`。
2. 完整传播仿真：构造 `A_theory * exp(1i * angle(p_exit_complex))`，用孔径内均匀理论幅值替换真实出口幅值，再传播到目标面并做空化固化预测。

核心判据：

- 若理想化出口幅值后的目标面声压和固化指标明显恢复，说明当前差结果主要来自真实相位板引入的出口幅值调制。
- 若恢复仍差，说明出口相位、离散投影或传播模型仍有主要误差来源。

输出目录：`phase_board_exit_repair_outputs/`
