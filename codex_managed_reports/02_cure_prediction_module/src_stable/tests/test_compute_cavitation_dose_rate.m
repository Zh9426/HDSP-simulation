...
function testSoftCloudWeightPreservesBaseDrive(testCase)
params = baseParams();
params.dose_cloud_radius_px = 1;
params.dose_cloud_floor = 0.10;
params.dose_cloud_weight = 0.25;
trigger = zeros(5, 5);
growth = zeros(5, 5);
trigger(3, 3) = 1.0;
growth(3, 3) = 1.0;

[dose_rate, components] = compute_cavitation_dose_rate(trigger, growth, params);

verifyGreaterThan(testCase, dose_rate(3, 3), 0.70);
verifyLessThan(testCase, dose_rate(3, 3), components.base_drive(3, 3));
end
