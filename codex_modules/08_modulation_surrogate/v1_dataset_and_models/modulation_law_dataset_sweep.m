...
run_meta = struct( ...
    ...
    'sample_seed', case_sample_seed(cfg.sample_seed, metrics.case_label), ...
...
export_exit_amp_surrogate_run( ...
    ...
    sim.complex_ratio_same, sim.complex_ratio_opposite, board.phase);
end
...
function seed_value = case_sample_seed(base_seed, case_label)
case_text = char(case_label);
case_weight = sum(double(case_text) .* (1:numel(case_text)));
seed_value = double(base_seed) + mod(case_weight, 1000000);
end
