...
metrics.target_p90_over_p50 = metrics.target_pressure_p90_pa / (metrics.target_pressure_p50_pa + eps);
metrics.target_p95_over_p50 = metrics.target_pressure_p95_pa / (metrics.target_pressure_p50_pa + eps);
metrics.target_peak_over_p50 = metrics.peak_target_pressure_pa / (metrics.target_pressure_p50_pa + eps);
...
metrics.target_band_loss = ...
    1.8 * max(0.90 - metrics.target_p05_over_p50, 0)^2 + ...
    2.4 * max(0.94 - metrics.target_p10_over_p50, 0)^2 + ...
    1.4 * max(metrics.target_p90_over_p50 - 1.08, 0)^2 + ...
    2.0 * max(metrics.target_p95_over_p50 - 1.12, 0)^2 + ...
    0.8 * max(metrics.target_peak_over_p50 - 1.45, 0)^2;
metrics.target_pressure_quality_score = ...
    2.0 * metrics.energy_efficiency + ...
    1.5 * metrics.target_uniformity_score + ...
    1.5 * metrics.target_p10_over_p50 + ...
    0.8 * metrics.target_p05_over_p50 - ...
    2.0 * metrics.target_band_loss - ...
    0.35 * max(metrics.target_p95_over_p50 - 1.12, 0) - ...
    0.20 * max(metrics.target_peak_over_p50 - 1.45, 0) - ...
    0.5 * metrics.dark_energy_leakage;
