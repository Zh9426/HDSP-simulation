...
metrics.target_band_loss = ...
    0.6 * max(0.76 - metrics.target_p05_over_p50, 0)^2 + ...
    1.0 * max(0.84 - metrics.target_p10_over_p50, 0)^2 + ...
    0.5 * max(metrics.target_p90_over_p50 - 1.20, 0)^2 + ...
    0.8 * max(metrics.target_p95_over_p50 - 1.30, 0)^2 + ...
    0.4 * max(metrics.target_peak_over_p50 - 1.75, 0)^2;
metrics.target_pressure_quality_score = ...
    ...
    0.8 * metrics.target_band_loss - ...
    0.25 * max(metrics.target_p95_over_p50 - 1.30, 0) - ...
    0.15 * max(metrics.target_peak_over_p50 - 1.75, 0) - ...
